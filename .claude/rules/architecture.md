...
| `storage.rs` | Shared `data_dir()` (XDG/HOME/APPDATA), atomic `write_atomic()`, and the `load_json()`/`save_json()` pair every persisted file goes through (`config`/`history`/`pb`). Loads degrade to `Default` on a missing/malformed file. Under `cfg!(test)` `data_dir()` returns a pid-keyed scratch dir — `App::new()` reads config and most state changes call `persist()`, so the real dir would be read and overwritten by the test suite. `write_atomic()`'s uniqueness guard is per-process only, not cross-process — two instances of the app running at once will have one save silently overwrite the other's (last writer wins); accepted as a low-risk trade-off for a single-user local TUI, not fixed |
